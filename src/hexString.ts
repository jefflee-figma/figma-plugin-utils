...
function hexAsciiToInt(char: number): number | undefined {
...
function hex1ToIntensity(char: number): number | undefined {
  const intVal = hexAsciiToInt(char);
  return intVal === undefined ? undefined : intVal / 15;
}

function hex2ToIntensity(big: number, little: number): number | undefined {
...
export function hexStringToRgba(hexStr: string): RGBA | undefined {
  if (hexStr[0] !== "#") {
    return undefined;
  }
